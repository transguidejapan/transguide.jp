...
function checkDependencies(): void {
  try {
    ...
    console.error(
      "Hugo is not installed or not found in PATH. Please install Hugo to proceed.",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    try {
      ...
      console.error(
        "Cargo (Rust package manager) is not installed or not found in PATH. Please install Rust and Cargo to proceed.",
      );
      process.exit(1);
    }
    try {
      ...
      console.error(
        "Failed to install obsidian-export. Please install it manually.",
      );
      process.exit(1);
    }
  }
...
function convertObsidianVault(): void {
  ...
  console.log(
    `Converting Obsidian vault from ${srcDir} to Hugo-compatible format...`,
  );
  try {
    execSync(`obsidian-export "${srcDir}" "${destDir}" --frontmatter=always`, {
      stdio: "inherit",
    });
